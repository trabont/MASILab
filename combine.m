...
% === USER SETTINGS ===
fullfitRoot = fullfile(pwd,'Processed','FullFit_HC');   % where all subject sub‑folders live
lines    = {'SL','L','G'};
rois    = {'GM','WM','CSF','LYMPH'};
accRoIs = setdiff(rois,'LYMPH'); % for the combined‐tissue sum, drop LYMPH
...
[nx,ny] = size(sample.(mapNames{1}));
nM = numel(mapNames);
HC_all_tissue = zeros(nx, ny, numel(lines), nF, nM);
...
for li = 1:numel(lines)
    ...
    for ri = 1:numel(accRoIs)
        ...
        for k = 1:nFiles
            S = load(fullfile(mf(k).folder,mf(k).name));
            for m = 1:nM
                HC_all_tissue(:,:,li,k,m) = HC_all_tissue(:,:,li,k,m) + S.(mapNames{m});
            end
        end
        % save per-ROI if needed
        data = zeros(nx,ny,nFiles,nM);
        ...
        save(fullfile(fullfitRoot,sprintf('HC_all_%s_%s.mat',line,roi)), 'data','mapNames','mf');
    end
end
save(fullfile(fullfitRoot,'HC_all_tissue.mat'),'HC_all_tissue','mapNames','lines','subjList');
...
sliceNames = cellfun(@(c) str2double(c{1}), sliceNamesTokens);
sliceNumbers = cellfun(@(c) str2double(c{1}), sliceTokens);
[xDim,yDim,nLines,nSlices,~] = size(HC_all_tissue);
...
for p=1:nPlots
    mi = tblIdx(p);
    switch mi
        case 1, plotData(:,:,:,:,p) = HC_all_tissue(:,:,:,:,1);
        case 2, plotData(:,:,:,:,p) = HC_all_tissue(:,:,:,:,2);
        case 3, plotData(:,:,:,:,p) = HC_all_tissue(:,:,:,:,3) .* HC_all_tissue(:,:,:,:,5);
        case 4, plotData(:,:,:,:,p) = HC_all_tissue(:,:,:,:,4) * 1e6;
    end
end
% global limits per plot
% global limits per plot
...
for li=1:nLines
    ...
    for p=1:nPlots
        ...
        cb = colorbar('eastoutside');
        cb.Layout.Tile = 'east';
        saveas(fig, fullfile(fullfitRoot, sprintf('HC_all_%s_%s_MAP.png', lineName, pname2)));
        close(fig);
    end
end

disp('Done plotting selected HC_all_tissue maps.');
